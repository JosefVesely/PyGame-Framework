...
class MainGame(GameState):
    ...
    def render(self, screen):
        screen.fill(pg.Color("lightgreen"))

        width, height = screen.get_width(), screen.get_height()

        text_state = self.font_large.render("Main game", True, pg.Color("darkgreen"))
        text_state_rect = text_state.get_rect(center=(width/2, height/2))
        screen.blit(text_state, text_state_rect)

        text_info = self.font_small.render("Press SPACE to change the game state", True, pg.Color("darkgreen"))
